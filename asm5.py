...
def encode_instruction(mnemonic, operands):
    ...
    if itype == "none":
        ...
        if reg == 0:
            ...
            raise ValueError("Поддерживается только " + ("push" if itype == "push" else "pop") + " рвх")
        ...
        code.extend(instr["code"])
        if reg < 8:
            code.append(0xD8 + instr["subop"] - 3 + reg)  # F7 D8..DF
        else:
            code.append(0x41)
            code.extend(instr["code"])
            code.append(0xD8 + instr["subop"] - 3 + (reg - 8))
        ...
        code.extend(base_code[:-1])  # \x48\xC1
        if reg < 8:
...
def parse_instruction_or_directive(tokens):
    ...
    if pass_num == 3:
        ...
        if itype == "reg_imm":
            ...
            position[current_section] += 2
        elif itype == "lea":
            position[current_section] += 10
        else:
            raise ValueError("Неподдерживаемые операнды для " + mnemonic + ": " + str(operands))
...
def parse(source):
    ...
    log("=== PASS " + str(pass_num) + " START ===")
    for line_num, line in enumerate(lines, start=1):
        log("[PASS " + str(pass_num) + "] Line " + str(line_num) + ": " + line.strip())
        try:
            tokens = tokenize_line(line)
            log_tokens(line_num, tokens)
            parse_tokens(tokens)
        except Exception as e:
            log("ERROR on pass " + str(pass_num) + ", line " + str(line_num) + ": " + str(e))
...
if len(sys.argv) != 2:
    print("Использование: python asm3_full.py <файл.квс>")
    sys.exit(1)
