...
INSTRUCTIONS = {
    ...
    "отрицать": {"code": b"\xF7", "subop": 3, "type": "unary"},
    "умножить": {"code": b"\xF7", "subop": 4, "type": "muldiv"},
    "умножить_знаковое": {"code": b"\xF7", "subop": 5, "type": "muldiv"},
    "разделить": {"code": b"\xF7", "subop": 6, "type": "muldiv"},
    "разделить_знаковое": {"code": b"\xF7", "subop": 7, "type": "muldiv"},

    # Логика
    "и": {"code": b"\x48\x21", "type": "reg_reg"},
    "или": {"code": b"\x48\x09", "type": "reg_reg"},
    "исключающее_или": {"code": b"\x48\x31", "type": "reg_reg"},
    "инвертировать": {"code": b"\xF7", "subop": 2, "type": "unary"},
    "проверить": {"code": b"\x48\x85", "type": "test"},

    # Адресация
    "загрузить_адрес": {"code": None, "type": "lea"},

    # Сдвиги и вращения
    "сдвиг_влево": {"code": b"\x48\xC1\xE0", "type": "shift_imm"},
    "сдвиг_вправо": {"code": b"\x48\xC1\xE8", "type": "shift_imm"},
    "сдвиг_арифметический_влево": {"code": b"\x48\xC1\xE0", "type": "shift_imm"},
    "сдвиг_арифметический_вправо": {"code": b"\x48\xC1\xF8", "type": "shift_imm"},
    "вращать_влево": {"code": b"\x48\xC1\xC0", "type": "rotate_imm"},
    "вращать_вправо": {"code": b"\x48\xC1\xC8", "type": "rotate_imm"},

    # Флаги
    "установить_перенос": {"code": b"\xF9", "type": "none"},
    "сбросить_перенос": {"code": b"\xF8", "type": "none"},
    "установить_направление": {"code": b"\xFD", "type": "none"},
    "сбросить_направление": {"code": b"\xFC", "type": "none"},

    # Прерывания
    "прервать": {"code": b"\xCD", "type": "int"},

    # === ДОБАВЛЕННЫЕ КОМАНДЫ ===
    # Расширение регистра
    "переместить_с_нулями": {"code": b"\x48\x0F\xB6", "type": "movzx"},
    "переместить_со_знаком": {"code": b"\x48\x0F\xBE", "type": "movsx"},

    # Обмен
    "обменять": {"code": b"\x48\x87", "type": "xchg"},

    # Флаги (полные)
    "втолкнуть_флаги": {"code": b"\x9C", "type": "none"},
    "вытолкнуть_флаги": {"code": b"\x9D", "type": "none"},

    # Условные переходы по CF
    "переход_если_перенос": {"code": b"\x0F\x82", "type": "jcc"},
    "переход_если_нет_переноса": {"code": b"\x0F\x83", "type": "jcc"},

    # Цикл
    "цикл": {"code": b"\xE2", "type": "loop"},

    # Альтернативный системный вызов
    "войти_в_систему": {"code": b"\x0F\x34", "type": "none"},

    # Строковые операции
    "переместить_байт": {"code": b"\xA4", "type": "none"},
    "переместить_слово": {"code": b"\xA5", "type": "none"},
    "сравнить_байты": {"code": b"\xA6", "type": "none"},
    "сканировать_байт": {"code": b"\xAE", "type": "none"},

    # Специальные команды
    "идентифицировать_процессор": {"code": b"\x0F\xA2", "type": "none"},
    "прочитать_счетчик": {"code": b"\x0F\x31", "type": "none"},
    "получить_управление": {"code": b"\x0F\x01\xD0", "type": "none"},
}
...
def parse_number_token(token):
    ...
    if s.startswith("0x"):
        return int(s, 16)
    raise ValueError("Недопустимое число: " + s)
...
def encode_instruction(mnemonic, operands):
    ...
    if itype == "none":
        ...
        if dst is None or src is None:
            raise ValueError("Неверные регистры: " + str(operands))
        code.extend(instr["code"])
        ...
        if reg is None:
            raise ValueError("Неверный регистр: " + str(operands))
        ...
        if reg == 0:
            ...
            # Поддержка всех регистров: 50+reg для push, 58+reg для pop
        ...
        code.extend(instr["code"])
        if reg < 8:
            code.append(0xC0 + instr["subop"] + reg)
        else:
            code.append(0x41)
            code.extend(instr["code"])
            code.append(0xC0 + instr["subop"] + (reg - 8))
        ...
        code.extend(instr["code"])
        if reg < 8:
            code.append(0xD0 + instr["subop"] - 2 + reg)
        else:
            code.append(0x41)
            code.extend(instr["code"])
            code.append(0xD0 + instr["subop"] - 2 + (reg - 8))
        ...
        code.extend(instr["code"])
        if reg < 8:
            code.append(0xE0 + instr["subop"] - 4 + reg)
        else:
            code.append(0x41)
            code.extend(instr["code"])
            code.append(0xE0 + instr["subop"] - 4 + (reg - 8))
        ...
        label = operands[1]
        if dst is None:
            raise ValueError("Неверный регистр назначения: " + operands[0])
        addr = parse_operand(label)
        if 0 <= dst <= 7:
            code.extend(b'\x48')
            ...
            code.extend(b'\x49')
            code.append(0xB8 + (dst - 8))
            code.extend(struct.pack("<Q", addr))
        else:
            raise ValueError("Регистр вне диапазона")
        ...
        if reg is None:
            raise ValueError("Неверный регистр: " + str(operands))
        base_code = instr["code"]
        code.extend(base_code[:-1])
        if reg < 8:
            code.append(base_code[-1] + reg)
        else:
            code.append(0x41)
            code.extend(base_code[:-1])
            code.append(base_code[-1] + (reg - 8))
    ...
    # === НОВЫЕ ТИПЫ ===
        ...
        src = REGISTERS.get(operands[1])
        if dst is None or src is None:
            raise ValueError("Неверные регистры для movzx")
        code.extend(instr["code"])
        modrm = 0xC0 | (src << 3) | dst
        code.append(modrm)
        ...
        src = REGISTERS.get(operands[1])
        if dst is None or src is None:
            raise ValueError("Неверные регистры для movsx")
        code.extend(instr["code"])
        modrm = 0xC0 | (src << 3) | dst
        code.append(modrm)
        ...
        src = REGISTERS.get(operands[1])
        if dst is None or src is None:
            raise ValueError("Неверные регистры для xchg")
        code.extend(instr["code"])
        modrm = 0xC0 | (src << 3) | dst
        code.append(modrm)
...
def parse(source):
    ...
    for line_num, line in enumerate(lines, start=1):
        log("[ПРОХОД 1] Line " + str(line_num) + ": " + line.strip())
        try:
    ...
    for line_num, line in enumerate(lines, start=1):
        log("[ПРОХОД 2] Line " + str(line_num) + ": " + line.strip())
        try:
            tokens = tokenize_line(line)
            parse_tokens(tokens)
        except Exception as e:
